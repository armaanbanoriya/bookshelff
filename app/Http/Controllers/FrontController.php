...
class FrontController extends Controller
{
    ...
    {
        // $catno=Category::find($id);
        $as= Product::find($id);

		// $catwith=Category::with('products')->get();
        $catwith=Product::where(['category_id'=>$as->category_id])->Where('id','!=',$id)->get();
		$otherproducts=Product::inRandomOrder()->get();
        return view('front.productdetail',compact('as','catwith','otherproducts'));
    }
    ...
    public function placeorder(Request $a)
    {
       ...
       $data->payment_method=$a->payment_method;
        ...
        {
            $lists = Order::with('orderproducts')->where('id',$order_id)->first()->toArray();
            $lists = json_decode(json_encode($lists),true);
            $productlists = User::where('email',$lists['useremail'])->first();
            ...
            Mail::send('front.emails.cod',$messageData,function($message)use($email){
        ...
        elseif($data['payment_method']=="Razorpay Gateway")
        {
            $user =$data['useremail'];
            $amount = $data['grand_total'];
            $order_id=$data['order_id'];
            return redirect('payment');
        }elseif($data['payment_method']=="PayU Money Gateway")
        {
...
// ////////////////////////////Paytm Gateway end
	...
	{
		$popcategories=Category::with('products')->find($id);
